...
exports.getSalaryStructure = async (req, res) => {
  try {
    ...
    const settings = await Settings.findOne() || { payroll: { hraPercent: 20, professionalTax: 200 } };
    const base = employee.baseSalary || 0;

    // Automated formulas based on professional standards
    res.json({
      baseSalary: base,
      allowances: {
        hra: Math.round(base * (settings.payroll.hraPercent / 100)),
        da: Math.round(base * 0.10), // Standard 10% DA
        travel: 1500,
        medical: 1250
      },
      deductions: {
        pf: Math.round(base * (settings.payroll.pfEmployerPercent / 100)),
        tax: Math.round(base * 0.05), // 5% Standard Tax
        insurance: 500
      }
    });
  } catch (err) {
...
const calculateSalaryDetails = async (employeeId, month) => {
  ...
  const allowances = {
    hra: Math.round(base * (payrollSettings.hraPercent / 100)),
    da: Math.round(base * 0.10), // 10% standard DA
    travel: 1500,
    medical: 1250,
    other: 0
  };
  ...
  // We check if "leave" in attendance is "unpaid" or "paid" via the Leave model.
  // If no Leave record exists for that date, we treat 'leave' as LOP (Unpaid).
  let lopDays = 0;

  // Basic LOP from half days
  lopDays += halfDayCount * 0.5;

  // Determine LOP for each 'leave' day marked in attendance
  ...
  const leaveDeduction = Math.round(lopDays * dailyRate);

  // 6. Deductions
  const deductions = {
    pf: Math.round(base * (payrollSettings.pfEmployerPercent / 100)),
